...
import { findings, format, metrics, parseLocalReport, percentile, useCases, type CheckReport, type EndpointConfig, type LocalReport, type TestMode, type TestPhase } from "@/lib/nettruth/model";
...
const phaseLabel: Record<TestPhase, string> = { connecting: "Connecting to measurement endpoint", latency: "Measuring response time", download: "Measuring download and loaded latency", upload: "Measuring upload and loaded latency", packetLoss: "Checking the UDP relay", complete: "Test finished" };
...
export function QuickCheck({ config }: { config: EndpointConfig }) {
  ...
  async function start() {
    ...
    setNotice(""); setRunning(true); setProgress(0); setPhase("connecting");
    const controller = new AbortController(); abortRef.current = controller;
    const initial = initialReport(mode, config, connection); setReport(initial); reportRef.current = initial;
    let interruption = "";
    ...
    track("NetTruth Test Started", { mode, endpoint: config.nodeOrigin ? "owned" : "reference" });
    try {
      const result = await runMeasurement(initial, config, controller.signal, state => { setReport(state.report); reportRef.current = state.report; setPhase(state.phase); setProgress(state.progress); });
      ...
      setNotice("The test could not finish. Please retry.");
      if (reportRef.current) setReport({ ...reportRef.current, status: "error" });
    } finally {
  ...
  return <div id="network-check" className="nt-shell">
    ...
    <section className="nt-console" aria-label="Network test controls and measurements">
      <div className="nt-control-row">
        <div><p className="nt-kicker">NETTRUTH NETWORK CHECK</p><h2>{running ? phaseLabel[phase] : completed ? report.status === "complete" ? "Your connection, measured." : "Partial results are available." : "Run a live connection test."}</h2><p className="nt-muted">{config.nodeOrigin ? config.nodeName : "Cloudflare reference edge"} <span className="nt-separator">/</span> {config.nodeOrigin ? "Operator-owned endpoint" : "HTTP measurements available"}</p></div>
        <button className={`nt-run ${running ? "nt-stop" : ""}`} onClick={running ? () => abortRef.current?.abort() : start}><span aria-hidden="true">{running ? "■" : "↗"}</span>{running ? "Stop test" : report ? "Run again" : "Run network test"}</button>
      </div>
    ...
    <p className="nt-privacy">Tests send traffic to {config.nodeOrigin ? config.nodeName : "Cloudflare"}, which sees your public IP. Results stay in this page unless you save, export, or send them. {config.nodeOrigin ? "" : "Cloudflare result reporting is disabled."} Site analytics records page activity and test status, not raw measurements.</p>
    ...
    <div className="nt-analysis-grid"><LatencyChart report={report} /><aside className="nt-summary"><p className="nt-kicker">READING THE RESULT</p><h2>{!report ? "Evidence before conclusions." : running ? "Collecting evidence…" : relevant.length ? `${relevant.length} finding${relevant.length === 1 ? "" : "s"} to investigate.` : "Review the measured coverage."}</h2><p>{!report ? "We look at responsiveness, consistency, and behavior under load. A missing measurement stays unknown." : `${measuredCount} of 6 performance metrics are available. ${report.loss.status !== "measured" ? "Packet loss remains unknown." : "Packet loss covers the tested UDP relay path."}`}</p><dl><div><dt>Idle p95</dt><dd>{format(m?.p95 ?? null)} ms</dd></div><div><dt>Download loaded</dt><dd>{format(m?.down ?? null)} ms</dd></div><div><dt>Upload loaded</dt><dd>{format(m?.up ?? null)} ms</dd></div></dl></aside></div>
