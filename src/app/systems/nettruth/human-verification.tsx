...
type Turnstile = { ready: (callback: () => void) => void; render: (container: HTMLElement, options: TurnstileOptions) => string | undefined; remove: (id: string) => void };
...
export function HumanVerification({ siteKey, attempt, signal, onVerified, onError, onCancel }: Props) {
  ...
  useEffect(() => {
    ...
    globals[loadCallback] = () => { if (mounted.current && !signal.aborted) setReady(true); };
    ...
    return () => { clearTimeout(timer); delete globals[loadCallback]; };
  ...
  useEffect(() => {
    ...
    api.ready(() => {
      if (finished || signal.aborted || !container.current) return;
      try {
  ...
  return <section className="nt-verification" aria-labelledby="nettruth-verification-title">
    ...
    {!ready ? <Script id={`nettruth-turnstile-${attempt}`} src={`https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit&onload=${loadCallback}`} strategy="afterInteractive" onReady={() => { if (mounted.current && !signal.aborted) setReady(true); }} onError={() => { if (mounted.current && !signal.aborted) onError(new Error("Verification could not load. Check your connection or browser blockers, then run the test again.")); }} /> : null}
  </section>;
}
