...
export function QuickCheck({ config }: { config: FleetConfig }) {
  ...
  return <div id="network-check" className="nt-shell">
    ...
    <section className="nt-console" aria-label="Network test controls and measurements">
      ...
      <div className="nt-server-detail"><p>{config.nodes.length === 1 ? `One test location is currently available: ${config.nodes[0].name}. Auto can switch regions once another node is deployed.` : `Auto compares ${config.nodes.length} configured nodes from your current connection before each run. No GPS access needed.`}</p>{report?.selection && <details><summary>Server selection for this run · {report.selection.mode === "auto" ? "Automatic" : "Manual"}</summary><ul>{report.selection.probes.map(probe => <li key={probe.nodeId}>{config.nodes.find(node => node.id === probe.nodeId)?.name || probe.nodeId}: {probe.status === "available" ? `${format(probe.medianRttMs)} ms selection probe median` : "Unavailable"}{probe.nodeId === report.selection?.selectedNodeId ? " · Selected" : ""}</li>)}</ul><p>Selection probes include HTTP and browser overhead. They are separate from the measured idle latency below.</p></details>}</div>
