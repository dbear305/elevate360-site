...
export function QuickCheck({ config }: { config: FleetConfig }) {
  ...
  return <div id="network-check" className="nt-shell">
    ...
    <div className="nt-title-row"><div><h1>Know your connection.</h1><p>Speed is one number. Get the evidence behind it.</p></div><Link href="/systems/nettruth/diagnostic" className="nt-text-link">Paid business diagnostics · from $750 ↗</Link></div>
    <p className="nt-service-boundary">Self-service testing and JSON export are currently available at no charge. Personal review and troubleshooting are paid services. <Link href="/systems/nettruth/diagnostic" className="nt-text-link">See scope and pricing</Link>.</p>
    <section className="nt-console" aria-label="Network test controls and measurements">
      ...
      <div className="nt-server-detail"><p>Measurement nodes: {config.nodes.map(node => node.name.replace("Elevate360 - ", "")).join(" and ")}. Results describe the tested route to the selected node. Nearby users can still experience routing delays or jitter.</p><p>{serverPreference !== "auto" ? "Manual selection tests only your chosen node." : config.nodes.length === 1 ? `Test location: ${config.nodes[0].name}.` : `Auto compares ${config.nodes.length} locations and chooses the lowest measured health-response time. No GPS needed.`}</p>{report?.selection && <details><summary>Server selection for this run · {report.selection.mode === "auto" ? "Automatic" : "Manual"}</summary><ul>{report.selection.probes.map(probe => <li key={probe.nodeId}>{config.nodes.find(node => node.id === probe.nodeId)?.name || probe.nodeId}: {probe.status === "available" ? `${format(probe.medianRttMs)} ms selection probe median` : "Unavailable"}{probe.nodeId === report.selection?.selectedNodeId ? " · Selected" : ""}</li>)}</ul><p>Selection probes include HTTP and browser overhead. They are separate from the measured idle latency below.</p></details>}</div>
    ...
    <section id="diagnostic" className="nt-diagnostic"><div><p className="nt-kicker">PAID NETWORK DIAGNOSTIC · FLORIDA &amp; NORTH TEXAS</p><h2>Get the evidence reviewed. Agree the next step.</h2><p>For businesses dealing with dropped calls, unreliable connections, or recurring slowdowns. Guided measurements, authorized configuration review, written findings, and a prioritized correction plan within an agreed scope.</p><span className="nt-price">Starts at $750 <span>· One location, one agreed problem · Remote investigation</span></span><p className="nt-fine">Scope, time allocation, and total price are agreed before work begins. Repairs, hardware, and ongoing support are quoted separately.</p></div><div><Link className="nt-button" href="/systems/nettruth/diagnostic" onClick={() => track("NetTruth Diagnostic Inquiry", { hasReport: Boolean(report) })}>See diagnostic scope<ActionIcon /></Link><a href={`mailto:contact@elevate360systems.com?subject=${encodeURIComponent("NetTruth Paid Diagnostic Scope Request")}&body=${encodeURIComponent(emailBody)}`} className="nt-text-link" onClick={() => track("NetTruth Diagnostic Email", { hasReport: Boolean(report) })}>Request scope by email</a><p className="nt-fine">Your results are not sent automatically. An inquiry does not book work or take payment.</p></div></section>
