...
Status (2026-09-10): owner-authorized public beta for the existing Elevate360 site. NYC and Dallas are deployed with working HTTPS and UDP relays. Daniel completed an extended Dallas run with all six metrics, automatic server selection, and 0 of 1,000 UDP messages lost. Raw-sample review confirmed the displayed arithmetic. Throughput closely matched one Cloudflare comparison, while loaded latency differed substantially; the cause has not been isolated. Controlled repeatability, capacity calibration, and broader browser/Windows coverage remain requirements before stronger accuracy or compatibility claims.
...
Start with one Quick Check. Confirm Auto selects a measured responsive location, the run completes, HTTP samples are present, packet loss is either measured with counts or explicitly unavailable, and JSON export succeeds. Fast links can lack loaded-latency samples with the current bounded payload plan; that is a calibration issue to resolve before accuracy claims. Never interpret missing metrics as a clean bill of health.
...
- Automatic server choice before each run, manual server selection, and server-selection evidence in exports. The inventory includes deployed NYC and Dallas nodes.
...
Auto selection needs real geographic coverage. NYC and Dallas are deployed and registered. Follow this procedure when adding another location; a label without a deployed server must never be added. Validate each node's capacity separately.
...
5. Add its actual `id`, `name`, and HTTPS `origin` to `nodes.json`, push a preview, and authorize the new preview origin on every registered node.
...
Before claiming broad Windows compatibility, run it on supported Windows 10/11 machines with and without admin access, with third-party endpoint protection, disabled/unknown optional features, and no active hardware adapter. Import the result, review it, and verify no network traffic or setting modifications occur. The Linux development environment cannot perform those Windows checks. The optional collector remains experimental and separate from browser performance coverage.

## Public beta verification and calibration backlog

Production uses the existing `main` branch and GitHub-to-Vercel integration. Confirm the production page shows both inventory nodes; build-time overrides can hide the inventory. Production origins are already in both nodes' allowed origins. Public beta release does not assert that the broader calibration backlog below is complete. Confirm live navigation, page rendering and one real test after deployment. Roll back to the prior known-good Vercel deployment if the public journey fails.
