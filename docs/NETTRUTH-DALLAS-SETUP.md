...
The defaults are `measure-dfw.elevate360systems.com` and `Elevate360 - Dallas (DFW)`. The script verifies the presented server key against the independently obtained fingerprint, checks existing known-host information, stages the complete source package, verifies its SHA-256 checksums, and invokes the regional installer. It never copies the private SSH identity or a relay secret. Enter the hardware-key PIN and touch the YubiKey locally when OpenSSH requests it.
